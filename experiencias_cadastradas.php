...
<?php
  ...
  $id = $_GET['id'];  
  $sql = "select * from EXPERIENCIAS where usuario_id = $id";
  ...
  echo"<table class='table table-sm'>
        <thead class='thead-light'>
          <tr align='center'>
            <th colspan=6 >Manutenção de Experiências</th>
          </tr>
          <tr>
            <th>Tipo</th>
            <th>Local</th>
  ...
  while($row = mysqli_fetch_array($limite)){
    ...
    echo "<td>" .$row['descricao'] ."</td>";
	  echo "<td><a class='btn btn-primary' href='alterar_cadastro_experiencia.php?id=$row[id]'> Editar </a></td>";
	  echo "<td><a class='btn btn-primary' href='delete_experiencia.php?id=$row[id]'> Excluir </a></td>";
    echo "</tr>";
	  echo "</tbody>";
  }
  echo"</table>";
