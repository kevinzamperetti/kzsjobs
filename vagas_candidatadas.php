...
<?php
  ...
  $id_usuario = $_GET['id'];  
  $sql = "select C.vaga_id, C.usuario_id, V.*
            from CANDIDATURAS C, VAGAS V
           where C.vaga_id = V.id and usuario_id = $id_usuario";
  ...
  echo"<table class='table table-sm'>
        <thead class='thead-light'>
          <tr align='center'>
            <th colspan=6 >Vagas que você está candidatado atualmente</th>
          </tr>
          <tr>
            <th>Id</th>
            <th>Título</th>
  ...
  while($row_vaga = mysqli_fetch_assoc($limite)){
    ...
    if($count == 0){
	   echo "<td width=10%><a class='btn btn-primary' href='candidatar_vaga.php?id=$row_vaga[id]&pagina=$pagina'> Candidatar-se </a></td>";
    }else{
     echo "<td width=10% > <font color='green'>Candidatado</font></td>";
	   echo "<td width=10%><a class='btn btn-primary' href='delete_candidatura.php?id=$row_candidatura[vaga_id]&idusu=$row_candidatura[usuario_id]&pag=vcand&pagina=$pagina'> Desfazer Candidatura </a></td>";
  }
  ...
  $proximo = $pc +1;
  if($pc>1){
    echo "<a class='btn btn-primary' href='?id=$id_usuario&pagina=$anterior'><- Anterior</a> ";
  }
  //echo "|";
  if($pc<$tp){
    echo "<a class='btn btn-primary' href='?id=$id_usuario&pagina=$proximo'>Próxima -></a>";
  }
