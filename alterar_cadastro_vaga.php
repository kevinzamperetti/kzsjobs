<?php
  ...
  $id = $_GET['id'];
  $sql_select = "select * from VAGAS where id = $id";
  ...
  <body>
    ...
    <main role="main" class="container">
      <div class="starter-template">
        <h2>Alteração de Vaga</h2>        
      </div>

      <!--Formulário de alteração de vaga-->
      <form class="col-sm-6 offset-md-4" action="update_vaga.php?id=<?php echo $row['id']; ?>" method="POST">
        <div class="form-row">
			    <div class="col-md-10 mb-3">
				    <label for="validationTitulo">Título:</label>
            <input type="text" class="form-control" id="validationTitulo" name="titulo" placeholder="Título da vaga" value="<?php echo $row['titulo']; ?>" required>
          </div>
				  <div class="col-md-10 mb-3">
            <label for="validationDescricao">Descrição:</label>
            <textarea class="form-control" id="validationDescricao" name="descricao" placeholder="Descrição da vaga" value="" rows="10" required> <?php echo $row['descricao']; ?> </textarea>
          </div>
				  <div class="col-md-10 mb-3">
        ...
        <a class='btn btn-primary' href='area_empresa.php' role='button'> Cancelar </a>
      </form>
